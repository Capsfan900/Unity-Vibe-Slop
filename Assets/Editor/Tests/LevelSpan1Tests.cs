...
namespace VibeGame1.Tests
{
    ...
    public class LevelSpan1Tests
    {
        ...
        public void WaterShoulderAndWestSentryKeepTheirAuditedOwnershipAndClearance()
        {
            ...
            Assert.That(Vector3.Distance(east.center, new Vector3(25f, 3.5f, 60f)), Is.LessThan(0.001f),
                "the east perch must remain a separate forward beat outside the cyan sun silhouette");
        }

        [Test]
        public void OldRailsWallsAndObelisksCannotReappear()
        {
            string[] removed = { "T1_Rail_L", "T1_Rail_R", "T1_Obelisk_W", "T1_Fallen_Obelisk",
                                 "T1_Wall_Start", "T1_Wall_Causeway", "T1_Wall_Landing" };
            foreach (string name in removed)
                Assert.IsFalse(def.platforms.Any(p => p.name == name), name + " re-cramps the open line");
            Assert.IsFalse(def.ramps.Any(r => r.name.StartsWith("T1_")), "T1 is landing-to-landing movement, not connector ramps");
        }
