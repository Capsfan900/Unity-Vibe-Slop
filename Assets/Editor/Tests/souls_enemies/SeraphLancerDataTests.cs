...
namespace VibeGame1.Tests
{
    ...
    public class SeraphLancerDataTests
    {
        ...
        {
            // Sandbox for now. The boss-roster plan's Stage 5 places him as the T1 realm boss (the
            // Spawn_Legendary_Ninja spawner's prefab key); when it does, this test becomes "is the T1
            // realm boss" -- it is NOT a "never" like the Judge's was.
            foreach (var guid in AssetDatabase.FindAssets("t:LevelDefinition", new[] { "Assets/Data" }))
            {
                string path = AssetDatabase.GUIDToAssetPath(guid);
                StringAssert.DoesNotContain("Legendary_SeraphLancer", File.ReadAllText(path),
                    path + " references the Seraph Lancer before Stage 5 placed him.");
            }
            foreach (var guid in AssetDatabase.FindAssets("t:LevelRegistry", new[] { "Assets/Data" }))
            {
                string path = AssetDatabase.GUIDToAssetPath(guid);
                StringAssert.DoesNotContain("Legendary_SeraphLancer", File.ReadAllText(path),
                    path + " lists the Seraph Lancer before Stage 5 placed him.");
            }
        }
