...
namespace VibeGame1
{
    ...
    public class FlaskAbility : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (combat.IsExecuting || combat.IsAttacking) return;
            if (health.Current >= health.Max - 0.01f) { AudioManager.Play(Sfx.Click, 0.4f, 0.6f); return; }
            if (!resources.UseFlask()) { AudioManager.Play(Sfx.Click, 0.4f, 0.6f); return; }
