...
The suite stands at 518 assertions. Every one of them proves a state machine. **None proves the game is
...
- **`DebugHarness` cannot reach the sandbox.** `FindSpawned("Spawn_GruntA")` falls back to the suffix
  `GruntA`, which never matches the sandbox's `Spawn_Grunt`, so `Run("parry")` there logs "no grunt A"
  and exits *reporting success* — the same silent-success shape that has bitten twice already.
...
- **`WeaponController.Awake` caches the viewmodel with an active-only `GetComponentInChildren`**, so
  hiding the viewmodel root and reloading the scene silently stops `Equip()` swapping weapon models.
...
Two intermittents remain, both staging races rather than game bugs. Neither has been seen to fail twice
in a row:

- `Flask_DrinkCoroutineAndInterruptOnHit` — observed `noHeal=False`: the heal lands before the interrupt.
...
## 5. Two remaining input-gated skips

`WandPedestal_FOpensMenu` (`InteractPressed`) and `WandPedestal_RCyclingStillWorks` (`WandCyclePressed`)
are polled from `InputReader` inside `Update()` with no script entry point. They want the same `Try*()`
treatment that closed the original seven skips — `TryInteract()` / `TryCycle()` called by `Update` with
the polled input, so behaviour is unchanged but reachable.
...
- **The sandbox scene holds the old ambient** until `7. Build Sandbox Scene` is run.
