...
3. Author contents. Each array element is one object:
   ...
   - **pedestals** — inscription altars. `groundPosition` is the floor it stands on; the plinth, crystal,
     glow and trigger are built from there. Underlying `Wand*` type names remain for serialized compatibility.
...
- **Water lines lie ON a deck** (sheet bottom within 6 cm of the top, inside the deck in plan, unit flow):
  the T1 fast slide deck (only an active grounded slide gets the no-decay speed floor; ordinary running and
  jumping keep normal traction/carry) and the
...
- **`WaterDef { name, center, size (x, 0.04, z), flowDirection, flowSpeed 6 }`.** A sheet of flowing water
  laid ON a platform's top (`center.y = top + size.y / 2`): while actively sliding it supplies a
  `groundSpeed × 1.35` floor, adds its flow and prevents slide decay. Running/standing use ordinary ground
  friction and jumps use ordinary air carry; trigger contact alone never throws the player.
...
## 4. Weapons, spellbook inscriptions and items
