...
| `Legendary_FlurryBrawler` — *The Flurry Brawler* | `Legendary_FlurryBrawler` | `Legendary_FlurryBrawler_Moveset` | `EnemyController` | **PROTOTYPE, sandbox pad only (x −22, z −26, the second row).** The roster's first FLURRY enemy: UNARMED, and where the Halberdier is REACH and the Revenant is a READ, this one is RHYTHM AND VOLUME — four-beat strings on a 0.73 s beat (above the Marionette's 0.69 s floor, which stays that fight's superlative), a 0.95 s hammerfist as the tempo break, an unblockable kick for the turtle and a 3.90 m shoulder charge for the kiter. Eight attacks, eight generated clips, every one named on the attack. **Imported ANIMATED body with GENERATED clips** (`Assets/Enemies/FlurryBrawler.fbx`). See §2b. |
