...
| `5. Build HUD` | `Editor/HudBuilder.cs` | `Assets/Prefabs/HUD.prefab` — bars, menus, test menu, EventSystem (`InputSystemUIInputModule`); carried-spell selection lives on the persistent book, not a duplicate bottom hotbar. Regenerates the glass sprites under `Assets/UI/Generated/` (`Editor/UiSprites.cs`: pane, tile, pill, track, shadow, edge light, sheen — 9-sliced PNGs, uncompressed, committed), then runs the styling passes in `Editor/HudExtensions.cs`: `ApplyFluidBars` (health + stamina → `FluidBarView` with `VibeGame1/UI/FluidBar`) and `ApplyPyreFire` (`PyreBar` → `FireBarView`, creating `Assets/Materials/UI/M_PyreFire.mat` from `Assets/Shaders/UI/FireBar.shader` on first run). Every number written in the passes. `HudGlassTests`, `FluidBarTests` and `FireBarTests` pin the shipped values. Also the settings INFO card built by `SettingsPanelKit` from `ControlsInfo.Text` (the key reference, off the playing HUD), and the F1 menu's INFO button. |
...
| `10. Build Parry Recording Stages` | `Editor/ParryRecordingStageFactory.cs` | Idempotently writes four isolated LevelDefinition assets under `Assets/Data/Levels/Recording/` (flat, downhill, ice/water, mixed) and builds each into its own `Assets/Scenes/ParryRecording_<Stage>.unity` through `LevelDefinitionBuilder`. The stage list lives in `Scripts/Debug/ParryRecordingStages.cs`; the main menu shows a developer-only, editor-only `RECORD …` row per built scene, loaded by path (not in the build scene list, so the exe never ships them). They contain no shooters; record a desired rhythm first, then generate an encounter module in Level Studio. |

The served dashboard binds to `127.0.0.1` and exposes one POST action only: **Open timing captures** creates and
opens the known LocalLow `timing-captures` directory. Static output displays the path but performs no local action.
| `8a. Rework Level_01 (parkour first)` | `Editor/LevelDefinitionAuthoring.cs` | Rewrites `Level_01_Level.asset` deterministically: broad T1–T3 decks plus restored shoulder walls/tower/lintels/obstacles, five connector ramps, the four-balloon T3 arc, water, Challenge Route flare data, isolated solar approaches, all shooter perches, five bounded projectile route-audit groups, and the explicitly progress-gated T0 opening volley. Idempotent; run it, then `8`, `Level Arc Report`, `Projectile Encounter Report` and both suites. `LevelTraversalTests` proves the same on a copy. |
...
The complete library is grouped into collapsible human task categories; only Start Here opens by default.
After granting developer access, **F1 → DEV DASHBOARD** opens this generated page from the Unity project.
Player builds do not bundle or launch the development manual.
